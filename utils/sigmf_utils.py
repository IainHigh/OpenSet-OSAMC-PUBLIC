...
def save_sigmf(i, q, config, idx):
    ...
    if "modnames" in config:
        f.add_annotation(
            0,
            config["n_samps"],
            metadata={
                "rfml_labels": {"modulations": config["modnames"]},
                "channel": {"type": config["channel_type"], "snr": config["snr"]},
            },
        )
    ...
    f.add_annotation(
        0, config["n_samps"], metadata={"rfml_labels": {"modclass": config["modname"]}}
    )
